...
for i, item in enumerate(textbox_data):
    item['id'] = f'reci3_num{i+1}_SV'
    item['trigger_id'] = f'reci3_num{i+1}_trigger'
    item['left'] = f'{BASE_SV_LEFT}px'
    item['top'] = f'{BASE_PV_TOP + i*56}px'

reci3_textbox_indicators = monitor_data + textbox_data
...
BASE_STEP_LEFT = 620
RECIPE_DEV_SET_POINT = 1200
RECIPE_DEV_OFFSET = 100
# ラベルの座標の再割り当て
for k in range(len(step_data)):
    item_list = step_data[k]
    for i, item in enumerate(item_list):
        item['id'] = f'reci3_num{k+1}_{i+1}_step'
        item['trigger_id'] = f'reci3_num{k+1}_{i+1}_trigger'
...
def create_recipe_edit3_layout():
    layout = []

    # レシピのオフセットの更新
    recipe_handler.update_edit_step_offset(offset=12)
    ...
    layout.append(
        html.Div(id='dummy-button-reci3', style={'display': 'none'})
    )
    ...
    # 8. ポップアップ用のレイアウト
    create_setting_modal(layout, '-reci3')
